...
func CreatePremiumMember(c *gin.Context) {
	...
	if tx := entity.DB().Where("id = ?", premiumMember.PremiumMemberPeriodID).First(&premiumMemberPeriod); tx.RowsAffected == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Preriod not found"})
		return
	}
...
func GetPremiumMember(c *gin.Context) {
	var premiummember entity.PremiumMember
	id := c.Param("id")
	if err := entity.DB().Preload("Member").Preload("MemberClass").Preload("PremiumMemberPeiod").Raw("SELECT * FROM premium_members WHERE id = ?", id).Find(&premiummember).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListPremiumMember(c *gin.Context) {
	...
	if err := entity.DB().Preload("Member").Preload("MemberClass").Preload("PremiumMemberPeiod").Raw("SELECT * FROM premium_members").Find(&premiummembers).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
